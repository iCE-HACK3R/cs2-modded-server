...
//#define MPBHOP_FIX // Uncomment to enable mpbhop fix (requires mpbhop plugin)
...
public plugin_init( ) {
	...
	// SQL - uses SQLite (file: addons/amxmodx/data/sqlite3/kreedz.sq3)
	g_hSqlTuple = SQL_MakeDbTuple( "", "", "", "kreedz" );

	// Auto-create leaderboard table
	new szQuery[ 512 ];
	formatex( szQuery, 511, "CREATE TABLE IF NOT EXISTS `%s` ( \
		...
		`Country` VARCHAR(8) DEFAULT 'n-a', \
		`Date` INTEGER NOT NULL DEFAULT 0, \
		`Weapon` VARCHAR(20) DEFAULT '', \
		`Cps` INTEGER NOT NULL DEFAULT 0, \
		`Gcs` INTEGER NOT NULL DEFAULT 0 \
	)", SqlTable );
	SQL_ThreadQuery( g_hSqlTuple, "HandleCreateTable", szQuery );
