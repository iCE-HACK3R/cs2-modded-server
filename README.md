...
> [!WARNING]
> The `release/css-1.0.373-production` branch is under production-hardening. CounterStrikeSharp and Custom Votes are checksum-locked, but remaining mode dependencies are still being migrated from the historical binary snapshot. See [Dependency supply chain](docs/SUPPLY_CHAIN.md) for the exact coverage and do not publish this branch as production-ready yet.
...
| [GameModeManager](https://github.com/iCE-HACK3R/GameModeManager)                      | `1.0.126`         | A CS2 server plugin that manages modes, settings, bot policy, and map rotation. Release validation is tracked separately until publication.                                                                |
...
Copy `.env.example` to `.env` before using Docker Compose. Keep `.env` local; it is ignored because it may contain Steam credentials.

- **If setting up for internet server:**

  Set the `STEAM_ACCOUNT` variable in the repository-root `.env` file.
  For workshop maps, set `API_KEY` in `.env`.
...
To securly pass .env vars to the container, _Kubernetes Secrets_ are used. Make a file called `cs2-secret.yaml`. Your keys need to be base64 encoded. Simply `echo "my_key" | base64` in your bash shell and then put them in the data section of your Secret manifest.
